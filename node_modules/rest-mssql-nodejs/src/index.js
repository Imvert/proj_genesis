...
class sqlRest {
      ...
      this.#pool.connect((err) => {
        if (err) {
          this.#error = {
            ...
            stack: err.stack,
            error_message: `Something went wrong when connecting to the database ${
        ...
        console.log(
          `Connection succesfully to the database ${this.#dbConfig.database}`
        );
      });
    } catch (err) {
      console.log(err);
    ...
    if (Array.isArray(paramsData)) {
      for (let param of paramsData) {
        try {
          request.input(
            param.name,
            this.#getParamProcedureType(param.type),
            param.value
          );
        } catch (e) {
  ...
  executeStoredProcedure = async (storedProcedure, schema, paramsData) => {
    ...
    const request = await this.#pool.request();
    const paramsProcedure = await this.#getProcedureParams(
      schema,
      storedProcedure
    );
    ...
    for (let param in paramsData) {
      try {
        ...
        request.input(
          paramsProcedure.data[param].name,
          paramsProcedure.data[param].type,
          paramsProcedure.data[param].value
        );
      } catch (e) {
        return this.#handleResponse(null, {
          stack: e,
    ...
    return request
      .execute(`${schema}.${storedProcedure}`)
      .then((res) => {
        return this.#handleResponse(res.recordsets);
      })
      .catch((err) => {
  ...
  #handleResponse = (data, error) => {
    let response = {
      ...
      itemsByPage:
        !error && data.length > 0 && data[0].itemsByPage
          ? data[0].itemsByPage
          : 0,
    };
